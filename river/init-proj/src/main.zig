...
const Options = struct {
    ...
    pub fn init(arena: mem.Allocator) @This() {
        ...
        return .{
            ...
            .status_bar = "i3bar-river",
            .xcursor_theme = "Bibata-Original-Classic 20",
        };
    }
    ...
    fn _screenshot_path(arena: mem.Allocator, HOME: []const u8) []const u8 {
        return fmt(
            arena,
            "{[HOME]s}/Pictures/screenshots/screenshot_{[date]s}.png",
...
const Run = struct {
    ...
    fn autostart(self: Run) void {
        ...
        const autostarts_commands = [_][]const u8{
            ...
            // "wlsunset -t 700 -l 55.45 -L 37.60",
            "wl-paste --watch cliphist store",
            "batsignal -b -e -p -w 35 -d 12 -f 85 -m 180 -D systemctl suspend",
            \\ riverctl keyboard-layout -options "grp:alt_shift_toggle,ctrl:nocaps" "us,ru" 
            \\
        };
    ...
    fn run_once(arena: mem.Allocator, program: []const u8, cmd: []const u8) void {
        ...
        posix.flock(
            ...
            error.FileLocksNotSupported => {
                ...
                std.log.err(
                    "{[err]s}: Failed to acquire lock on file {[lockfile]s}",
                    .{ .err = @errorName(err), .lockfile = lockfile },
                );
                std.process.exit(1);
            },
    ...
    fn configure_input(self: Run) void {
        const device = "pointer-2-10-TPPS/2_Elan_TrackPoint";
    ...
    fn mappings(self: Run) void {
        ...
        const normal_mappings = Map{
            .normal = &.{
                ...
                // TODO: is not working due to language swiching
                ...
                // NOTE : makes no sense with 0.5 ratio
                // .{
                //     .mod = &.{ .Super, .Shift },
                //     .key = .E,
                //     .cmd = "zoom",
                ...
                // Take screenshot
                .{ .mod = &.{ .Super, .Control }, .key = .S, .cmd = 
                \\spawn 'grim -g "$(slurp)" - | swappy -f -'
                },
                .{
                    .mod = &.{.Super},
                    .key = .H,
    ...
    fn rivertile(self: Run) void {
        run(self.arena, fmt(
            self.arena,
            "riverctl default-layout rivertile",
            .{},
        ));
        run(self.arena, fmt(self.arena, "rivertile -view-padding 0 -outer-padding 0 -main-ratio 0.5 -main-count 1", .{}));
        // posix.execvpeZ("rivertile", &.{null}, &.{null}) catch unreachable;
        //        posix.execvpeZ("rivertile", &.{
        //            "-view-padding 0 ",
        //            "-outer-padding",
        //            " 0",
        //            "-main-location",
